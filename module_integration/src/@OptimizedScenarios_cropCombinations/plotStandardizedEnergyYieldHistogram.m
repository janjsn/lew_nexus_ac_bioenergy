...
outputFolder = [outputFolder '/Optimal solutions energy yield histograms/'];
export_source_data = 1;

filename = [outputFolder obj.scenarioDescription_string '_' timestamp '.tif'];
if export_source_data == 1
    filename_source_data = [outputFolder obj.scenarioDescription_string '_' timestamp '.mat'];    
end
...
temp = filename_source_data{1};
for cells = 2:length(filename_source_data)
    temp = [temp filename_source_data{cells}];
end
filename_source_data = temp;
